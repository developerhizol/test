...
<i>Заявка закрыта по инициативе пользователя.</i>""",
    ...
    # Отправляем пользователю запрос на отзыв (БЕЗ клавиатуры и промежуточного сообщения)
    try:
        await bot.send_message(
            chat_id=user_id,
...
async def handle_admin_message(message: Message):
    # Проверяем, не находится ли админ в состоянии ожидания отзыва
    from aiogram.fsm.context import FSMContext
    ...
    if current_state == TicketStates.WAITING_FOR_FEEDBACK:
        # Админ находится в состоянии ожидания отзыва - обрабатываем отдельно
...
async def handle_user_message(message: Message):
    ...
    if current_state == TicketStates.WAITING_FOR_FEEDBACK:
        # Пользователь находится в состоянии ожидания отзыва - обрабатываем отдельно
        await handle_feedback_comment(message, state)
        return
...
async def main():
    ...
    print(f"📢 Канал для заявки: {SUPPORT_CHAT_ID}")

...
class TicketStates(StatesGroup):
    ...
    WAITING_FOR_FEEDBACK = State()  # Новое состояние для отзыва
...
async def cmd_start(message: Message, state: FSMContext):
    await state.clear()  # Очищаем любое состояние при старте
...
async def handle_respond_button(callback: CallbackQuery, state: FSMContext):
    ...
    # Обновляем сообщение в канале - убираем кнопку "Ответить"
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="❌ Закрыть", callback_data=f"close_{ticket_id}")]
    ])
    ...
    # Отправляем сообщение админу в личку - ВАЖНОЕ ИЗМЕНЕНИЕ!
    try:
        await bot.send_message(
...
<b>Для завершения диалога нажмите «❌ Закрыть» под сообщением в канале поддержки.</b>

<i>Отправьте ваше первое сообщение пользователю:</i>"""
        )
    except Exception as e:
        logger.error(f"Ошибка при отправке сообщения админу: {e}")
        # Если не удалось отправить в личку, отправляем в текущий чат
        await callback.message.answer(
            ...
            f"<i>Для завершения диалога нажмите «❌ Закрыть» под сообщением в канале.</i>"
        )
...
async def handle_close_button(callback: CallbackQuery, state: FSMContext):
    if not can_access_admin(callback.from_user.id):
        await callback.answer("⛔ У вас нет прав для этого действия.", show_alert=True)
        return
    ...
    # Проверяем, брал ли этот админ заявку в работу
    if in_work_by and in_work_by != callback.from_user.id:
        await callback.answer("❌ Вы не можете закрыть заявку, взятую не вами.", show_alert=True)
        conn.close()
        return
...
Спасибо что выбрали BotHost 😊""",
    ...
    try:
        ...
        # Убираем кнопки после закрытия
        await callback.message.edit_reply_markup(reply_markup=None)
    except Exception as e:
        logger.error(f"Ошибка при обновлении сообщения: {e}")
    
    # Очищаем состояние ответа админу
    await state.clear()
...
async def handle_skip_feedback(callback: CallbackQuery, state: FSMContext):
    ...
    # Обновляем запись о том, что отзыв не оставлен
    conn = sqlite3.connect('bothost_support.db')
    cursor = conn.cursor()
...
async def handle_feedback_comment(message: Message, state: FSMContext):
    ...
    # Отправляем отзыв в канал поддержки
    try:
        await bot.send_message(
            chat_id=SUPPORT_CHAT_ID,
            text=f"""📝 <b>НОВЫЙ ОТЗЫВ ПО ЗАЯВКЕ #{ticket_number}</b>
...
# Обработка ответов пользователя админу (когда пользователь отвечает на сообщение от бота)
...
async def handle_user_reply_to_admin(message: Message):
    ...
    if message.reply_to_message.from_user.id == (await bot.me()).id:
        # Ищем заявку пользователя, которая в работе
        conn = sqlite3.connect('bothost_support.db')
        cursor = conn.cursor()
...
async def handle_unknown(message: Message, state: FSMContext):
    ...
    if current_state:
        # Если пользователь в состоянии ожидания отзыва
        if current_state == TicketStates.WAITING_FOR_FEEDBACK.state:
            await handle_feedback_comment(message, state)
        else:
            await message.answer(
                "🤔 Неизвестная команда в текущем режиме.\nИспользуйте кнопки меню или /cancel для отмены.",
                reply_markup=get_main_keyboard(message.from_user.id)
            )
    else:
        # Если пользователь просто отправил неизвестную команду
        await message.answer(

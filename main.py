...
async def cmd_cancel(message: Message, state: FSMContext):
    ...
    if can_access_admin(message.from_user.id):
        await message.answer(
...
async def handle_admin_message(message: Message):
    ...
    # Пропускаем, если это кнопка главного меню
    if message.text in ["🎫 Создать тикет", "📋 Мои заявки", "📊 Статистика", 
                       "❓ Частые вопросы", "📌 Контакты", "👑 Админ-панель"]:
        return
    ...
    # Пропускаем, если это команда админ-панели (они обрабатываются отдельно)
    if message.text in ["📊 Общая статистика", "✅ Решенные заявки", "❌ Нерешенные заявки",
                       "👥 Назначить поддержкой", "📢 Рассылка", "« Назад в меню"]:
        return
    ...
    if not ticket:
        # Если нет активной заявки, проверяем, не в состоянии ли админ
        current_state = await dp.storage.get_state(user=message.from_user.id)
        
        if current_state:
            # Если есть состояние, пропускаем
            conn.close()
            return
        
        # Это обычное сообщение админа без активной заявки - игнорируем
        await message.answer("У вас нет активной заявки для общения. Возьмите заявку в канале поддержки.")
        conn.close()
        return
...
@dp.message(F.content_type.in_([
    ...
    ContentType.VOICE, ContentType.DOCUMENT, ContentType.ANIMATION,
    ContentType.VIDEO_NOTE, ContentType.STICKER
]))
async def handle_user_message(message: Message):
    # Пропускаем команды и кнопки главного меню
    if message.text in ["/start", "/cancel", "🎫 Создать тикет", "📋 Мои заявки", 
                       "📊 Статистика", "❓ Частые вопросы", "📌 Контакты", 
                       "« Назад в меню", "❌ Отменить создание", "« Назад к категориям"]:
        return
    
    # Пропускаем админов
    if can_access_admin(message.from_user.id):
        # Проверяем, не в состоянии ли админ
        current_state = await dp.storage.get_state(user=message.from_user.id)
        if not current_state:
            return
        # Если админ в состоянии (например, ждет ID для добавления поддержки), пропускаем
        return
    ...
    if not ticket:
        # Если нет активной заявки, проверяем, не в состоянии ли пользователь
        current_state = await dp.storage.get_state(user=message.from_user.id)
        if current_state:
            # Если пользователь в состоянии (например, создает заявку), пропускаем
            conn.close()
            return
        
        # Это обычное сообщение пользователя без активной заявки
        await message.answer("❌ <b>У вас нет активной заявки.</b>\nДля создания заявки нажмите «🎫 Создать тикет»")
        conn.close()
        return
    ...
    if success:
        await message.answer("✅ <b>Ваше сообщение отправлено поддержке.</b>", 
                            reply_markup=get_ticket_work_keyboard(is_admin=False))
    else:
...
async def show_admin_stats(message: Message):
    ...
    closed_tickets = cursor.fetchone()[0] or 0
    ...
    support_count = cursor.fetchone()[0] or 0
    ...
    avg_response = int(avg_response_result) if avg_response_result else 0
    ...
    efficiency = int((closed_tickets/total_tickets*100) if total_tickets > 0 else 0)
...
⏱️ <b>Среднее время ответа:</b> {avg_response} мин.
...
async def process_broadcast(message: Message, state: FSMContext):
    ...
    status_message = await message.answer(f"📢 <b>Начинаю рассылку для {len(users)} пользователей...</b>")
...
async def process_admin_id(message: Message, state: FSMContext):
    ...
    try:
        ...
        logger.error(f"Ошибка при получении информации о пользователе {new_admin_id}: {e}")
        username = ""
        first_name = ""
...
<b>Начните работу:</b> @{(await bot.me()).username}"""
        ...
        logger.error(f"Ошибка при отправке уведомления новому сотруднику: {e}")
...
async def show_statistics(message: Message):
    ...
    efficiency = int((closed_tickets/user_tickets*100) if user_tickets > 0 else 0)
...
<b>Эффективность поддержки для вас:</b> {efficiency}%""",
        reply_markup=get_main_keyboard(message.from_user.id)
    )

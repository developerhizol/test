...
async def show_solved_tickets(message: Message):
    ...
    cursor = conn.cursor()
    cursor.execute('''
        SELECT ticket_number, username, category, priority, created_at 
    ...
    for ticket in tickets:
        ticket_num, username, category, priority, created_at = ticket
...
async def handle_respond_button(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await callback.message.answer(
            ...
            f"Просто отправляйте сообщения - они будут пересылаться пользователю.\n\n"
...
async def handle_close_button(callback: CallbackQuery):
    ...
    if in_work_by and in_work_by != callback.from_user.id:
        await callback.answer("❌ Не вы взяли заявку, не вам и отклонять.\nПошёл нахуй 😊", show_alert=True)
        conn.close()
        return
...
{message.text or message.caption or '[файл]'}

───────────────────────────
<i>Ответьте пользователю напрямую через бота.</i>"""
                    )
                    ...
                    cursor.execute('''
                        INSERT INTO responses (ticket_id, admin_id, response_text, created_at)
                        VALUES (?, ?, ?, ?)
                    ''', (ticket_id, message.from_user.id, message.text or message.caption or '[файл]', datetime.now()))
                    conn.commit()
    ...
    # Если это не ответ на сообщение от бота
    if message.text:
        await message.answer(

...
<i>Заявка закрыта по инициативе пользователя.</i>""",
    ...
    # Отправляем пользователю запрос на отзыв БЕЗ клавиатуры
    try:
...
Спасибо что выбрали BotHost! 😊""",
            reply_markup=ReplyKeyboardRemove()  # Убираем ВСЕ reply-клавиатуры
        )
        
        # Отправляем inline-клавиатуру с оценкой отдельным сообщением
        await bot.send_message(
            chat_id=user_id,
            text="<b>Выберите оценку:</b>",
...
@dp.message(F.text == "« Назад в меню")
async def handle_admin_back_to_menu(message: Message, state: FSMContext):
    await state.clear()
    await cmd_start(message)
...
async def handle_admin_message(message: Message):
    ...
    state_data = await storage.get_data(key=storage_key)
    current_state = state_data.get('state')
    
    if current_state == TicketStates.WAITING_FOR_FEEDBACK:
        # Админ находится в состоянии ожидания отзыва - обрабатываем через состояние
        return
...
async def handle_user_message(message: Message):
    ...
    if current_state == TicketStates.WAITING_FOR_FEEDBACK:
        # Пользователь находится в состоянии ожидания отзыва
        return

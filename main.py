...
def init_db():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS tickets (
            ...
            waiting_feedback BOOLEAN DEFAULT FALSE
        )
    ''')
...
def get_main_keyboard(user_id: int) -> ReplyKeyboardMarkup:
    """Основная клавиатура для пользователя (без админ-панели)"""
    keyboard = [
...
def get_admin_inline_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="✅ Решенные заявки", callback_data="admin_solved")],
        [InlineKeyboardButton(text="❌ Нерешенные заявки", callback_data="admin_unsolved")],
        [InlineKeyboardButton(text="👥 Назначить поддержкой", callback_data="admin_assign")],
...
async def close_ticket_for_user(ticket_id: int, user_id: int, ticket_number: str, closer_name: str, is_user_closing: bool = False):
    ...
    # Обновляем статус заявки и отмечаем, что ожидается отзыв
    cursor.execute('UPDATE tickets SET status = "closed", response_time = ?, can_user_close = FALSE, waiting_feedback = TRUE WHERE id = ?', 
...
<i>Заявка закрыта по инициативе пользователя.</i>""",
    ...
    # Отправляем пользователю запрос на отзыв БЕЗ reply-клавиатуры
    try:
        await bot.send_message(
            chat_id=user_id,
...
def check_waiting_feedback(user_id: int) -> bool:
    """Проверяет, ожидает ли пользователь отзыва"""
    conn = sqlite3.connect('bothost_support.db')
    cursor = conn.cursor()
    cursor.execute('SELECT COUNT(*) FROM tickets WHERE user_id = ? AND status = "closed" AND waiting_feedback = TRUE AND feedback_provided = FALSE', 
                   (user_id,))
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    # Проверяем, не ожидает ли пользователь отзыва
    if check_waiting_feedback(message.from_user.id):
        await message.answer(
            """📝 <b>У вас есть незавершенный отзыв</b>
──────────────────────────
Пожалуйста, завершите оценку заявки, чтобы продолжить использование бота.""",
...
async def handle_skip_feedback(callback: CallbackQuery, state: FSMContext):
    ...
    conn = sqlite3.connect('bothost_support.db')
    cursor = conn.cursor()
    cursor.execute('UPDATE tickets SET feedback_provided = TRUE, waiting_feedback = FALSE WHERE id = ?', (ticket_id,))
    conn.commit()
    conn.close()
...
<b>Хорошего дня! 😊</b>"""
    ...
    # После отзыва возвращаем клавиатуру
    user_id = callback.from_user.id
    await bot.send_message(
        chat_id=user_id,
        text="✅ <b>Отзыв завершен. Возвращаемся в главное меню.</b>",
...
async def handle_feedback_comment(message: Message, state: FSMContext):
    ...
    conn = sqlite3.connect('bothost_support.db')
    cursor = conn.cursor()
    cursor.execute('UPDATE tickets SET feedback_comment = ?, feedback_provided = TRUE, waiting_feedback = FALSE WHERE id = ?', 
...
@dp.callback_query(F.data == "admin_solved")
async def handle_admin_solved(callback: CallbackQuery):
    if not can_access_admin(callback.from_user.id):
        await callback.answer("⛔ Нет доступа", show_alert=True)
        return
...
@dp.callback_query(F.data == "admin_unsolved")
async def handle_admin_unsolved(callback: CallbackQuery):
    if not can_access_admin(callback.from_user.id):
        await callback.answer("⛔ Нет доступа", show_alert=True)
        return
...
@dp.callback_query(F.data == "admin_assign")
async def handle_admin_assign(callback: CallbackQuery, state: FSMContext):
    if not is_admin(callback.from_user.id):
        await callback.answer("⛔ Только владельцы могут назначать поддержку", show_alert=True)
        return
...
async def handle_admin_back(callback: CallbackQuery, state: FSMContext):
    ...
    admin_status = "👑 Владелец" if is_admin(callback.from_user.id) else "🛠️ Поддержка"
...
# ========== ОБРАБОТКА АДМИН ID (из состояния) ==========
...
<b>Начните работу командой:</b> /admin"""
        )
    except:
        pass
...
# ========== ОБРАБОТКА РАССЫЛКИ (из состояния) ==========
...
async def process_broadcast(message: Message, state: FSMContext):
    ...
    if not users:
        await message.answer("❌ <b>Нет пользователей для рассылки.</b>", reply_markup=get_admin_inline_keyboard())
        await state.clear()
        return
...
async def handle_user_message(message: Message):
    # Проверяем, не ожидает ли пользователь отзыва
    if check_waiting_feedback(message.from_user.id):
        # Если пользователь пишет что-то во время ожидания отзыва
...
async def main():
    ...
    print("✅ Reply-клавиатуры скрываются при ожидании отзыва")
    print("✨" * 40)

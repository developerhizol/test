            ...
            try:
                await bot.send_message(
                    ...
                    text=f"📭 <b>Пользователь завершил обращение</b>\n\n"
                         f"🎫 <b>Номер заявки:</b> #{ticket_number}\n"
                         f"👤 <b>Пользователь:</b> {closer_name}\n"
                         f"🕒 <b>Время:</b> {datetime.now().strftime('%d.%m.%Y %H:%M:%S')}\n\n"
                         f"<i>Заявка закрыта по инициативе пользователя.</i>",
    ...
    try:
        ...
        await bot.send_message(
            ...
            text=f"✅ <b>Заявка #{ticket_number} закрыта</b>\n"
                 f"🔒 <b>Закрыл:</b> {closer_type}\n"
                 f"👤 <b>Имя:</b> {closer_name}\n"
                 f"🕒 <b>Время:</b> {datetime.now().strftime('%d.%m.%Y %H:%M:%S')}"
        )
    except Exception as e:
...
async def handle_admin_message(message: Message):
    ...
    if message.text == "❌ Закрыть заявку":
        # Этот случай обрабатывается отдельным хендлером
        return
...
async def handle_user_message(message: Message):
    ...
    if message.text in ["/start", "🎫 Создать тикет", "📋 Мои заявки", "📊 Статистика", 
                       ...
                       "❌ Отменить создание", "« Назад к категориям", "❌ Завершить обращение"]:

...
async def forward_media_to_user(message: Message, user_id: int, ticket_number: str, from_admin: bool = True):
    ...
    sender = "поддержки" if from_admin else "пользователя"
...
<i>Просто отправьте сообщение в этот чат - оно будет переслано.</i>"""
            ...
            # Отправляем отдельное сообщение с пояснением
            await bot.send_message(
                chat_id=user_id,
                text=f"""📨 <b>Ответ по заявке #{ticket_number}</b>

<b>Видеосообщение от {sender}</b>
...
<b>Голосовое сообщение от {sender}</b>
...
async def handle_respond_button(callback: CallbackQuery):
    ...
    cursor.execute('UPDATE tickets SET in_work_by = ?, status = "in_progress", admin_chat_id = ? WHERE id = ?', 
...
async def handle_admin_message(message: Message):
    # Проверяем, не кнопка ли "❌ Закрыть заявку"
    if message.text == "❌ Закрыть заявку":
        return  # Этот случай обрабатывается отдельным хендлером
    ...
    if not ticket:
        # Если нет активной заявки, проверяем, не команда ли это
        if message.text not in ["👑 Админ-панель", "📊 Общая статистика", "✅ Решенные заявки", 
                               "❌ Нерешенные заявки", "👥 Назначить поддержкой", "📢 Рассылка", 
                               "« Назад в меню", "📋 Мои заявки", "📊 Статистика", "❓ Частые вопросы", 
                               "📌 Контакты", "🎫 Создать тикет"]:
    ...
    if in_work_by != message.from_user.id:
        await message.answer("❌ Эта заявка взята другим сотрудником.")
        conn.close()
        return
    ...
    if success:
        await message.answer(result_message, reply_markup=get_support_work_keyboard())
    else:
        await message.answer(result_message, reply_markup=get_support_work_keyboard())
...
async def handle_user_message(message: Message):
    ...
    cursor.execute('''
        SELECT id, ticket_number, in_work_by 
        FROM tickets 
    ...
    if not ticket:
        ...
        # Можно отправить подсказку или проигнорировать
        # await message.answer("У вас нет активных заявок. Создайте новую через «🎫 Создать тикет».")
        conn.close()
        return
    ...
    if not admin_id:
        await message.answer("❌ Ваша заявка еще не взята в работу.")
        conn.close()
        return
...
async def handle_close_from_chat(message: Message):
    # Проверяем права

...
async def process_issue(message: Message, state: FSMContext):
    ...
    ticket_number = f"BH-{date_part}-{count:04d}"  # Исправлено: date_part вместо date-part
...
# ВАЖНОЕ ИЗМЕНЕНИЕ: Добавляем фильтр для обработки ВСЕХ сообщений в состоянии WAITING_FOR_RESPONSE_TO_USER
...
async def handle_unknown(message: Message, state: FSMContext):
    ...
    if current_state:
        if current_state == TicketStates.WAITING_FOR_FEEDBACK.state:
            ...
            # Проверяем, не находится ли пользователь в состоянии ответа
            if current_state == TicketStates.WAITING_FOR_RESPONSE_TO_USER.state:
                # Передаем сообщение в обработчик для админов
                await handle_admin_message_to_user(message, state)
            else:
                await message.answer(
